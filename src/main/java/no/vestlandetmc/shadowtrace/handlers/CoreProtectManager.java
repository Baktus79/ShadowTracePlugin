...
public class CoreProtectManager {
	...
	private final List<Object> blocksOverworld = Arrays.asList(
			Material.COAL_ORE,
			Material.COPPER_ORE,
	...
	private final List<Object> blocksNether = Arrays.asList(
	...
	public void findOres(Player player, OfflinePlayer target, String time) {
		...
		Bukkit.getScheduler().runTaskAsynchronously(ShadowTrace.getPlugin(), () -> {
			final List<String[]> lookup = coreProtectAPI.performLookup(convertToSeconds(time), username, null, blocksOverworld, null, action, 0, null);
			final List<String> messageList = new ArrayList<>();
	...
	public void findOresNether(Player player, OfflinePlayer target, String time) {
		...
		Bukkit.getScheduler().runTaskAsynchronously(ShadowTrace.getPlugin(), () -> {
			final List<String[]> lookup = coreProtectAPI.performLookup(convertToSeconds(time), username, null, blocksNether, null, action, 0, null);
			final List<String> messageList = new ArrayList<>();
